...
Simple QuickMatch for whole word matching was implememented in OpenCL and tested on Intel integrated graphics cards. Full regex matching requires the use of complex pointer data structures which are not fully supportedo on OpenCL. Hence the full-regex matching using Thompson's NFA was implemented using CUDA and tested on the GHC cluster.
